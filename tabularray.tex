...
\begin{longtblr}[
  caption = {Templates for Headers and Footers}
]{
  hlines = {white},
  row{odd}={brown8},row{even}={gray8},row{1}={fg=white,bg=purple2,font=\bfseries\sffamily},
  column{2} = {co=1},
  ...
  \V{contfoot}     & continuation text in the foot, normally ``Continued on next page'' \\
  \V{conthead}     & continuation text in the head, normally ``(Continued)'' \\
...
\begin{codehigh}
\DefTblrTemplate{contfoot}{normal}{Continued on next page}
\SetTblrTemplate{contfoot}{normal}
\DefTblrTemplate{conthead}{normal}{(Continued)}
...
\begin{codehigh}
\DefTblrTemplate{contfoot}{default}{Continued on next page}
...
At this moment, foreground colors and font commands are supported.
And in most cases the keys \verb!fg! and \verb!font! can be omitted.
